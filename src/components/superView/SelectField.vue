...
<template>
  <div class="grid-one p-all-1 grid-gap-5">
    <div class="row-start-center">
      ...
      <a-select allow-clear :trigger-props="{ preventFocus: false }" @change="changeValue" v-model="props.modelValue" :placeholder="$t('please_input')+ props.title" :options="getCanOption" @clear="changeValue('')" :field-names="{ value: 'id', label: 'name' }"
        show-header-on-empty :show-extra-options="false">
        <template #header v-if="canAdd">
            <a-input-search
            :placeholder="$t('create_field_msg')"
              search-button
...
<script setup>
  ...
  async function sureAdd() {
    ...
    if (newFieldName.value) {
      ...
      Message.info(t("ts_field_name_no_empty"));
    }
